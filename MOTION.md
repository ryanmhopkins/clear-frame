...
The app uses the free tabs-sliding, accordion, checkbox-check, toggle, and toast recipes from [transitions.dev](https://transitions.dev). Their CSS and per-recipe variables are copied verbatim into `motion.css`; `styles.css` provides the app colors and dimensions. `motion.js` adapts the documented tab and accordion orchestration to the existing vanilla JavaScript interface. Native checkboxes remain the accessible inputs, with decorative animated SVG checks.
